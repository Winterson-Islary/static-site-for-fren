		...
		<section className=" w-[40rem] h-full flex flex-col justify-center">
			...
			<main>
				<form onSubmit={handleSubmit} className="flex flex-col gap-5 mb-5">
					<input name="username" type="text" className="border-b-2 w-full text-black py-2 focus:px-1 bg-transparent" placeholder="Name" />
					<input name="subject" type="hidden" value={"New submission from {{username}}"} />
					<input id="email" name="email" type="email" className="border-b-2 w-full text-black py-2 focus:px-1 bg-transparent" placeholder="Email" />
					<ValidationError prefix="email" field="email" errors={state.errors} />
					<textarea id="message" name="message" className="border-b-2 w-full text-black py-2 focus:px-1 bg-transparent" placeholder="How can we help?" />
					<ValidationError prefix="message" field="message" errors={state.errors}/>
					<button className="bg-black text-white rounded-[5px] p-1 text-2xl font-bold font-playfair hover:bg-slate-800" disabled={state.submitting}>Submit</button>
				</form>
